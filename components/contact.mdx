---
title: "Contact Page: Arcade High Score Entry and Guestbook"
...
Open `data/mockData.js` and update the `guestbook` array (exported as `g`). Reorder, add, or remove entries as desired. Rank is derived from position in the array, so sort entries by descending score for correct display.
...
Score values in the guestbook are purely decorative. Feel free to assign any numbers that fit the arcade aesthetic — the important visual effect is the high-score formatting with commas.
